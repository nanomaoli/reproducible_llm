        ...
        for batch_size in 8 16 32; do
            for passk in 4; do
                ...
                python vllm_passk.py --model meta-llama/Llama-3.1-8B-Instruct \
                    ...
                    --max_tokens 2048 \
                    --passk $passk \
                    --exp_name 2A100_PASS_${passk}_${task}_${dtype}_bs_${batch_size} > logs/2A100_PASS_${passk}_llama_${task}_${dtype}_bs_${batch_size}.log 2>&1
        ...
        for batch_size in 8 16 32; do
            for passk in 4; do
                ...
                python vllm_passk.py --model deepseek-ai/DeepSeek-R1-Distill-Llama-8B \
                    ...
                    --max_tokens 32768 \
                    --passk $passk \
                    --exp_name 2A100_PASS_${passk}_${task}_${dtype}_bs_${batch_size} > logs/2A100_PASS_${passk}_deepseek-llama_${task}_${dtype}_bs_${batch_size}.log 2>&1
...
for task in gpqa_diamond math500; do
    ...
    for dtype in bfloat16; do
        for batch_size in 32; do
            ...
            python vllm_main_followup_exp.py --model deepseek-ai/DeepSeek-R1-Distill-Qwen-7B \
                ...
                --batch_size $batch_size \
                --max_tokens 32768 \
                --exp_name 2A100_CATCHUP_${task}_${dtype}_bs_${batch_size} > logs/2A100_CATCHUP_deepseek-qwen_${task}_${dtype}_bs_${batch_size}.log 2>&1
